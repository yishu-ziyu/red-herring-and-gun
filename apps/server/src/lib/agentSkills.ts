...
export const AGENT_SKILLS: AgentSkill[] = [
  {
    ...
    body: [
      ...
      "2. 区分：可核查事实 / 价值判断 / 无法证伪的修辞。微博级短句、免票、要建地铁、打架、P图配文只要原句作了判断，就是可核查事实，不得当八卦丢掉。",
  ...
  },
  {
    ...
    body: [
      "Skill · 双向证据",
      "1. 同时考虑支持与反驳；找不到反证 ≠ 命题为真。无检索来源不得判真/假。",
      "2. 单源搜索摘要不能直接升格为结论。模型记忆不是核查。",
      "3. unresolvedEvidenceGaps 必须写清「还缺什么可核材料」。",
      "4. 引用 URL/标题必须来自输入 search360 / atomSearches，禁止编造。已公开辟谣必须点到来源。",
    ].join("\n"),
  },
  {
...
export function selectAgentSkills(opts: {
  ...
  for (const skill of AGENT_SKILLS) {
    ...
    // 有 claimTypes 的 skill 需要 type 也匹配。
