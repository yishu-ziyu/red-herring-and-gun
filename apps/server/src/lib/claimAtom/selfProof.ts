...
export const SELF_PROOF_SYSTEM_PROMPT = [
  ...
  "3. 本闸门只判「忠实」（原句是否直接声称），不判「可核查性」。立场/价值/规范/预测型原子若原句直接声称了该立场或断言，即使后续可能判为不可核查，也应判 supported=true——是否可核查由后续排除层另行处置，不在本判断范围。",
  "4. 原句本身就是一条短流传判断时，不得因「太琐碎」「像八卦」判 supported=false。",
  "5. 课文/百科长文里的背景铺垫、修辞、无独立真假断言的切片（如「又称」「蜿蜒于」「这句话曾被写进」），判 supported=false。只留可单独核对的断言。",
