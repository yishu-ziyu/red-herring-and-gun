...
describe("postProcessHandoffFinalReport (Prompt A+F)", () => {
  it("strips drama and injects uncertainty without prepending the raw claim", () => {
    ...
    expect(String(report.conclusion)).not.toContain("流传说法是");
    expect(String(report.conclusion)).not.toContain("中央已经决定下月起取消退休金");
  ...
  it("原输入含 URL 时也不把 URL 前缀塞进结论第一句", () => {
    const result = postProcessHandoffFinalReport(
      {
        verdictType: "false",
        conclusion: "「隔夜菜亚硝酸盐超标百倍」与实测数据不符，「直接致癌」也不成立。",
        summaryForPublic: "两条核心主张均被反证。",
        canSay: ["两条主张都有可点开的反证来源"],
        cannotSay: ["不能推出所有隔夜菜都完全安全"],
      },
      "https://weibo.com/status/50891234 隔夜菜亚硝酸盐超标百倍直接致癌？真的假的？",
    );
    expect(String(result!.report.conclusion)).toBe(
      "「隔夜菜亚硝酸盐超标百倍」与实测数据不符，「直接致癌」也不成立。",
    );
    expect(String(result!.report.conclusion)).not.toContain("https://");
  });
