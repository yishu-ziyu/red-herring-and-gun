...
export function PromptInput({
  ...
  // The material is plain text; input controls must never add instructions to it.
  ...
  // Collapse the demo model popover whenever the menu closes.
  useEffect(() => {
    if (!menuOpen) {
  ...
  const send = () => {
    if (disabled || submitDisabled || busy || composing.current) return;
    onSubmit?.(value);
  ...
  // Remove an attachment only after its exit animation.
      ...
      <div ref={frameRef} className={styles.frame} data-prompt-frame>
        ...
        <div className={styles.editorWrap}>
          <div
            ...
            onCompositionEnd={() => { composing.current = false; syncFromEditor(); }}
          />
