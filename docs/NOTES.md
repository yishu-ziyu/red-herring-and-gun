2026-09-11 展示层 A 方案（结论区分成两层）机器项通过、待人工验收（独立验收官 ACCEPT）：按 `docs/evals/2026-09-11-conclusion-lede-layers.md` 契约，把结论区从「一整段 24px 粗体」恢复成用户 2026-09-06 已批准的 mode3 参照那种「一句判断 + 一段解释」两层。数据层 `packages/core/src/investigation/build.ts`（镜像 `mvp/server/src/lib/investigation/build.ts`，`cmp` 字节一致）新增确定性分层函数：`directAnswer` 语义与 400 字截断不动，新增可选字段 `verdictLead` 装判断句、启用 schema 里预留但从未被赋值的 `rationale` 装解释层；顶层 `boundaries` 不再并入各命题的 `claim.boundary`，只承载整调查级 `report.causalBoundary`，消除首屏同一句话出现两次。前端 `ConclusionHero.tsx` 判断句用 24px/700、解释层 15px/400 并加 `data-gp-rationale` 测试钩子，两字段缺省时回退渲染 `directAnswer`（旧快照与 core 单句路径不受影响）。复述句判定经三轮修订收敛为「剥离引导词与连接词后必须与原句完全相同，删除全部比例阈值」，判据宁严不宽、判不出来就保留 —— 前三轮的比例判据分别误丢过判断句「原句站不住。」和「原句 + 新信息」的句子，属内容损失。门禁：core build.lede 9 项、mirror 5 项、goldenPath 97 项、server 691 过 / 1 跳过全绿；mvp 102 文件 1139 过 / 1 跳过；根 npm test 四 workspace 全绿；mvp build 与 mvp/server tsc 通过。人评项（首屏观感与 `docs/design/reference-pack/screenshots/desktop-mode3-hybrid.png` 并排裁决）待人工验收，本轮取证为 FIXTURE 渲染（`/?fixture=complete` 驱动真实组件树与真实 build 输出），不是真实 SSE。附带发现：5199 预览的 vite 进程连续运行三十多小时后 `?fixture=*` 走查路径不再启动，用同一份配置重启后恢复，属进程陈旧而非产品缺陷，同日已重启。
