...
2026-09-06 PR #67 人工 Review 第二轮：#61 方向（Mode 3 / Hybrid + Quiet Editorial）通过，本轮只修真实性与稳定性，未开 #62–#66。① Mobile 390 加号/发送 hit target 44×44，视觉圆仍 28（`PromptInput.module.css`）；② `.gp-conflict` / `.gp-gaps` / `.gp-image-origin` 去掉 inset+全边框+radius 卡片，改为 editorial annotation；Interrupted 保留功能 surface；③ Token 事实源改为生产 CSS + Production Spec（`--gp-canvas #fcfcfd`、`--gp-surface-paper` / `--gp-surface-inset`、motion 140/240/280ms），不以旧 PR 描述回改 CSS；④ `:focus-visible` 与 reduced-motion 收紧到 `.gp-shell`；⑤ PromptInput 嵌入契约改为 `--prompt-frame-*` CSS 变量 + `data-prompt-frame`，删除 `[class*="frame"] !important`；⑥ E3 改为 Playwright computed-style；⑦ 768 `scrollWidth <= clientWidth` 真测；⑧ 新增 `desktop-investigating-conflict-gap.png`（production Golden Path + deterministic fixture，不是真实 SSE）。等待人工复审，不 merge。
