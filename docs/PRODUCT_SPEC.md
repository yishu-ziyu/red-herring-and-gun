...
**2. 复杂调查，简单理解。** 严谨不意味着难用。低摩擦、友善、漂亮是产品要求，不是发布后的美化项。视觉体验属于产品门禁：视觉验收与功能验收同等作为发布门禁；不先解决信息架构再留到 polish，也不先换皮再解决信息架构。视觉层级、留白、动效和交互节奏必须帮助用户理解调查逻辑、降低摩擦、产生探索欲。
...
- 不展示虚构的角色对话或运维面板。Mission Control 不是产品脸，多 Agent 角色不是核心 UI；多 Agent 可以传递证据、提出独立意见和具体质询，帮助用户理解判断，但默认不以 Agent 术语出现在前台（见第二节）。
