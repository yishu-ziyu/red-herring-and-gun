...
用户面前的语言分两层。五词——**说法、出处、判断、追问、历史**（见 `docs/ROADMAP.md`）——是顶层产品语言与导航骨架，不是封闭词表：调查必需的证据语义——**支持、反驳、仅相关、尚缺、争议**——必须能直接出现在用户面前，不因五词被藏掉。实现层词汇（Agent 名、provider、tool call、token、pipeline、模型推理、内部 verdict enum、调试信息）仍然一律不许出现。
