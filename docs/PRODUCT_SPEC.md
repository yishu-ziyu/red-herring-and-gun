...
2. **RumorDetector（拆题）**：同一套模型（开发默认 MiniMax-M3）被叫来只做分类，不判真假。拆出原句里真实存在的判断，给每条写 `type` 和 `verifiable`。另一次自证调用丢掉原句没说过的。详见本节「类型谁标」。
...
拆题、核查、写报告，开发默认都是 **MiniMax-M3**。不是 Mimic，也没有另雇一个更强的模型来认类型。
...
证据冲突有第二意见（主判 MiniMax-M3，二审 step-3.7-flash）。**类型标签没有第二模型复核。** 模型整步挂掉会 fail-open（整句当可核查继续搜）。流传说法被标错立场，现在由强制可核查闸兜住。
