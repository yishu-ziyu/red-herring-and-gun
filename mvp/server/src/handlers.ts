...
async function callParallelSearchProviders({
  ...
  const settled = await Promise.allSettled(
    providers.map(async (provider) => ({
      provider,
      result: await callSearchProvider({ env, provider, query, model, refProm }),
    }))
  );
  ...
  return {
    ...
    traceText: `并行搜索完成：${successes.map(({ provider, result }) => `${getProviderLabel(provider)} ${result.sources?.length ?? 0} 条`).join("，")}。${failures.length ? `失败：${failures.join("；")}` : ""}`,
