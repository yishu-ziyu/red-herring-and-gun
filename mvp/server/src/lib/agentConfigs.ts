...
export const AGENT_CONFIGS: AgentConfig[] = [
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "用户可见判断必须出现这四字之一：能信 / 不能信 / 只能信一部分 / 还查不清。写在 conclusion 首句。对应 verdictType：true→能信，false→不能信，mixed_misleading→只能信一部分，unverified→还查不清。",
      "conclusion / summaryForPublic 按同一骨架写，一句一事，2–5 句：",
      "1. 判断：能信 / 不能信 / 只能信一部分 / 还查不清。",
      "2. 对象：原句在断言什么。",
      "3. 材料：现有出处支持或反驳了什么，带 [n]。",
      "4. 边界：仍不能推出什么，或还缺哪条出处。",
      "先给判断，再给对象，再给材料，最后给边界。不要用「然而」「截至目前」「总的来说」起句。不要写建议用户做什么。不要写「我们检索了」。",
      "recommendation 只写判断，不要写转不转、先别转发、行动建议。",
