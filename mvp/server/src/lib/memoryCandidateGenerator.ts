...
/** Loose search snapshot — pipeline search360Result is not always full Search360Response. */
...
/** Pipeline / runtime step shape used only for unresolved-question extraction. */
export interface MemoryStepSnapshot {
  evidenceBundle?: { unresolvedQuestions?: string[] };
...
interface BuildMemoryCandidatesInput {
  ...
  searchResult?: MemorySearchSnapshot | null;
}
...
export function buildMemoryCandidatesFromRun({
  ...
  const unresolvedQuestions = uniqueStrings([
    ...
    ...steps.flatMap((step) => getStringArray(step.output, "unresolvedEvidenceGaps")),
    ...steps.flatMap((step) => getStringArray(step.output, "unresolvedQuestions")),
  ]).slice(0, 20);
  const base = {
  ...
  for (const candidate of buildSourceReputationCandidates(runId, base, searchResult ?? undefined)) {
...
function buildSourceReputationCandidates(
  ...
  const grouped = new Map<string, NonNullable<MemorySearchSnapshot["sources"]>>();
...
function buildReasoningPatternPayload(
  finalReport?: Record<string, unknown>
): ReasoningPatternMemoryPayload | null {
...
function sourceReputationChangesNextAction(payload: SourceReputationMemoryPayload, occurrenceCount = 0) {
  const scoreSpread =
    payload.observedScores.length > 1
...
function sourceDomain(source?: { domain?: string; url?: string }) {
