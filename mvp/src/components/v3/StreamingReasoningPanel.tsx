...
export function StreamingReasoningPanel({ session }: StreamingReasoningPanelProps) {
  ...
  // 跟踪"是否见过本 session 在跑"——仅当 from-running→all-done 转换时自动折叠，
  // 避免 mock 数据一开始就是 completed 时也被折叠。
  ...
  useEffect(() => {
    ...
    if (prevSessionRef.current !== sessionKey) {
      ...
      sawRunningRef.current = false;  // 新 session: 重新计数 running 转换
  ...
  // 监听 running → all-done 转换：流式输出完自动折叠，让用户看到完整结论
  useEffect(() => {
    if (!session) return;
    const hasRunning = session.stages.some((s) => s.status === "running");
    if (hasRunning) {
      sawRunningRef.current = true;
    } else if (
      sawRunningRef.current &&
      session.stages.length > 0 &&
      session.stages.every((s) => s.status === "completed" || s.status === "error")
    ) {
      // 跑过 + 全部结束 → 自动折叠
      setIsExpanded(false);
      sawRunningRef.current = false;  // 避免重复触发
    }
  }, [session]);
