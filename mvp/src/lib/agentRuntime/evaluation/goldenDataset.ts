...
export const goldenDataset: GoldenCase[] = [
  ...
  },
  {
    ...
    difficulty: "trap",
    domain: "health",
    description: "经典相关≠因果陷阱：把跨国健康差异直接归因于单一生活习惯",
    expectedClaimType: "causal",
    expectedAgentSequence: ["rumor_detector", "fact_checker", "source_validator", "alternative_explanation_searcher", "counter_evidence_grader", "report_composer"],
    expectedVerdictType: "mixed_misleading",
    expectedCredibilityRange: [10, 35],
    goldenRationale: "观察性跨国关联不能证明'喝红酒→预防心脏病'。饮食结构、医疗体系、吸烟率等混杂因素均可解释差异；过量饮酒本身有心血管风险。可承认'有观察性关联讨论'，但不得写成已证实因果预防方案。",
    traps: ["把相关性当成因果关系", "忽视混杂因素与替代解释", "把跨国生态学研究当个体因果证据"],
  },
  {
    id: "RUMOR-012",
    ...
    difficulty: "trap",
    domain: "health",
    description: "时序巧合 + 已辟谣因果链：接种后事件被错误归因",
    expectedClaimType: "causal",
    expectedAgentSequence: ["rumor_detector", "fact_checker", "source_validator", "alternative_explanation_searcher", "counter_evidence_grader", "report_composer"],
    expectedVerdictType: "false",
    expectedCredibilityRange: [0, 15],
    goldenRationale: "疫苗与自闭症的因果关联已被大规模研究否定；接种后短期发热是已知可预期反应，不能用'先接种后症状'推出病因。应判定为虚假因果归因。",
    traps: ["把时间先后当成因果关系", "用个案轶事推翻人群级反证", "把已知副作用夸大成无关严重结局的病因"],
  },
  {
    id: "RUMOR-013",
    ...
    difficulty: "trap",
    domain: "health",
    description: "生态谬误 + 世俗趋势：把地区级关联写成产品防癌因果",
    expectedClaimType: "causal",
    expectedAgentSequence: ["rumor_detector", "fact_checker", "source_validator", "alternative_explanation_searcher", "counter_evidence_grader", "report_composer"],
    expectedVerdictType: "mixed_misleading",
    expectedCredibilityRange: [10, 35],
    goldenRationale: "地区癌症死亡率下降可被筛查普及、诊疗进步、人口结构变化等解释，不能从'推广后下降'推出'保健品防癌'。可承认存在时间先后与生态关联讨论，但不得写成已证实的个体防癌因果。",
    traps: ["把生态关联当成个体因果", "忽视医疗进步与筛查等混杂/替代解释", "用'推广后下降'的时序当作疗效证明"],
  },
  {
    id: "RUMOR-014",
    ...
    difficulty: "trap",
    domain: "health",
    description: "选择偏倚 + 基率谬误：重症聚集导致的假因果",
    expectedClaimType: "causal",
    expectedAgentSequence: ["rumor_detector", "fact_checker", "source_validator", "alternative_explanation_searcher", "counter_evidence_grader", "report_composer"],
    expectedVerdictType: "false",
    expectedCredibilityRange: [0, 15],
    goldenRationale: "医院死亡率更高主要因为重症与临终患者集中就医，属于选择偏倚与基率问题，不能推出'就医导致死亡风险升高'。应判定为虚假因果归因。",
    traps: ["忽略重症患者更易就医的选择偏倚", "把基率差异当成治疗有害的证据", "用场所死亡率比较替代治疗效应评估"],
  },
];
