/**
 * reportReviewer.ts — AI Agent Book 提议者～审核者（proposer-reviewer）
 *
 * ReportComposer 是 proposer；本模块是确定性 reviewer（非再调 LLM）：
 * 检查契约字段、证据链、边界表达，并做最小可修复修补。
 * 目标：挡住「过早宣布完成」与「空壳报告」。
 */
...
/**
 * 审核并最小修复 report_composer 输出。
 * previousOutputs：前序 step 的 output，用于忠实度粗检。
 */
export function reviewAndRepairReport(
  ...
  // 8) summaryForPublic 不得比 conclusion 更绝对（粗规则）
