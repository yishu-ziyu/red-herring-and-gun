...
function getMimoConfig() {
  const apiKey =
    ...
    "";
  const baseUrl = (
    (typeof import.meta.env !== "undefined" && import.meta.env.MIMO_BASE_URL) ||
    (typeof process !== "undefined" && process.env.MIMO_BASE_URL) ||
    "https://token-plan-cn.xiaomimimo.com/anthropic"
  ).replace(/\/$/, "");
  const model =
    (typeof import.meta.env !== "undefined" && import.meta.env.MIMO_MODEL) ||
...
function buildSherlockSystemPrompt(): string {
  ...
  return [
    ...
    "10. canSay 列出基于搜索结果能信的事项。",
    "11. cannotSay 列出基于搜索结果不能信的事项。",
    "",
