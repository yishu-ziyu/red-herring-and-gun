...
function normalizeCredibilityScoreForLabel(score: number, label: string) {
  const bounded = Math.max(0, Math.min(100, score));
  return isLowCredibilityLabel(label) && bounded > 50 ? 100 - bounded : bounded;
}

function formatCredibilityLine(payload: ClosureReportPayload) {
  const credibilityScore = normalizeCredibilityScoreForLabel(payload.credibilityScore, payload.credibilityLabel);
  const confidenceScore = isLowCredibilityLabel(payload.credibilityLabel) ? 100 - credibilityScore : credibilityScore;
  return `判断置信度 ${confidenceScore}%，原信息可信度 ${credibilityScore}%`;
}
...
export function buildRebuttalCardMarkdown(payload: ClosureReportPayload): string {
  ...
  return [
    ...
    `## ${card.title}`,
    "",
    `**结论**：${card.verdict}（${formatCredibilityLine(payload)}）`,
    "",
    "### 核心要点",
...
export async function shareVerification(payload: ClosureReportPayload): Promise<ShareLogEntry> {
  const shareText = [
    ...
    `结论：${payload.credibilityLabel}（${formatCredibilityLine(payload)}）`,
    payload.summaryForPublic,
  ].join("\n");
