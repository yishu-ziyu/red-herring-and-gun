...
const STAGE_TEMPLATES: readonly StageTemplate[] = [
  ...
  },
  {
    ...
    chunks: [
      ...
      { type: "result", content: "若支持与反证冲突，系统会标记为存疑；只有关键来源缺口才进入人工复核。" },
      { type: "divider", content: "---" },
  ...
  },
  {
    ...
    chunks: [
      ...
      { type: "result", content: "命题 A：根据来源覆盖情况标记为可判断、存疑或人工复核。" },
      { type: "result", content: "命题 B：若只找到转述而无原始来源，标记为存疑。" },
      { type: "result", content: "命题 C：若多个来源一致反驳，进入结论推理；若关键数据缺口无法补齐，再进入人工复核。" },
      { type: "divider", content: "---" },
