...
export function providerHasCredentials(env: Record<string, string>, provider: string): boolean {
  ...
  if (id === "codex") return Boolean(envValue(env, "CODEX_BIN"));
  return false;
}
...
export async function dispatchSingleProvider({
  ...
  if (provider === "codex") {
    return await callCodexAgent({ codexBin, model, systemPrompt, userContent, responseSchema, maxTokens, env });
  }
  ...
  for (const provider of providerOrder) {
    ...
    if (provider === "codex") {
      ...
      const out = await runOne("codex-cli", model, (sys, user) =>
        callCodexAgent({ codexBin, model, systemPrompt: sys, userContent: user, responseSchema, maxTokens, env })
      );
      if (out.ok) {
        return out.result;
      }
