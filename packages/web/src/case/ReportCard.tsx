...
import { CitedText, StanceMarks, TierBadge } from "./Citation.js";
...
export function ReportCard(props: {
  ...
  return (
    <article className="report-card" data-running={running ? "true" : "false"}>
      {running || props.aborted ? <p className="status-line">{latestStatus(current, running, props.aborted)}</p> : null}
      {conclusion ? (
...
function ClaimLines(props: { current: Case; report: Report | undefined; flashClaim?: string | null }) {
  ...
  if (props.current.claims.length === 0) return null;
  return (
    <ol className="report-claims">
